    ...
    methods
        function main(Algorithm,Problem)
            ...
            while true
                ...
                CVs=sum(max(Population.cons,0),2);
                fs=CVs<=0;
                if ~isempty(fs)
                    break;
                end
            ...
            Arch=[];
            [FrontNo,CrowdDis] = CalFitness(Population,Problem.N);
            Arch=[Arch Population(fs)];
            CPS = [];
            Metric=[];
            Metric_HV=[];
            igds=[];
...
%                     objs1=Population.objs+2*t1;
                    ...
                    Arch=UpdateArch([Arch,Population],Problem,Problem.N);
                    t=t+1;
                    initPops=[initPops;Population];
                end
                MatingPool = TournamentSelection(2,Problem.N,FrontNo,-CrowdDis);
                Offspring  = OperatorGA(Problem,Population(MatingPool));
                [Population,FrontNo,CrowdDis] = EnvironmentalSelection([Population,Offspring],Problem.N,NF);
                Arch=UpdateArch([Arch,Population],Problem,Problem.N);
                Optimums=Problem.Optimums;
                Optimum_t=cell2mat(Optimums(t,2));
